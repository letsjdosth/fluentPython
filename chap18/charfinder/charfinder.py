...
class UnicodeNameIndex:
    ...
    def load(self, chars=None):
        #인덱스 불러오기. 인수를 받으면 해당 문자에 해당하는 인덱스를 만들며, 인수가 없으면 기존 피클파일이 있으면 가져오고, 아니면 일정 범위 문자에 대해 새로 만든다.
        self.index = None
        if chars is None:
            try:
    ...
    def build_index(self, chars=None):
        ...
        for char in chars:
            ...
            for word in tokenize(name):
                ...
                #index[대문자 문자명]이 있으면 그 value를, 없으면 빈 셋을 value로 돌려주고 해당 value에 실제 문자 char를 합집합 연산함
        self.index = index
    ...
    def describe(self, char):
        #실제문자를 받으면 유니코드포인트/실제문자/문자명 의 네임드튜플을 돌려줌
    ...
    def describe_str(self, char):
        #(보통 CharDescription 등 네임드튜플을 받아) 받은 인수(3요소 튜플)를 유니코드포인트\t실제문자\t문자명 스트링으로 포매팅
        return '{:7}\t{}\t{}'.format(*self.describe(char))

    def find_description_strs(self, query, start=0, stop=None): 
    #쿼리가 오면 discribe_str형식을 반환하는 이터레이터를 반환한다
        for char in self.find_chars(query, start, stop).items:
